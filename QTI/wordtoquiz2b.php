...
if (!isset($_POST['text'])) {
	...
	echo '<p>Copy-paste in the quiz below.  To be processed correctly:</p><ul><li>Do not use auto-numbering</li><li>Number the questions in the form 1. question</li><li>Letter the options in form A. choice<li>After the last choice, specify the answer in the form: Answer: B</li></ul>';
...
$html = preg_replace('/<strong>\s*<\/strong>/','',$html);
$html = preg_Replace('/<br\s+\/>/','',$html);
$html = preg_replace('/^(<strong>)?\d+\.\s*(<\/strong>)?\s*/m', ':Q:', $html);
$html = preg_replace('/<strong>\w\.\s*<\/strong>/','',$html);
$html = preg_replace('/^[A-G]+\.\s*/m', ':o:', $html);
$html = preg_replace('/^\s*Explanation:.*?Type:/sm','Type:',$html);
$html = preg_replace('/^\s*Type:.*?$/m','',$html);
$html = preg_replace('/^\s*Answer:\s*([A-G]).*?$/m',':o:$1',$html);
$pts = explode(':Q:',$html);
array_shift($pts);
...
foreach ($pts as $c=>$pt) {
	...
	$prompt = preg_replace('/^(<strong>)?\s*\d+(\.|\))\s*(<\/strong>)?\s*/','',trim($pp[0]));
	$prompt = htmlentities($prompt);
	$items = array();
	$corrects = array();
	$last = array_pop($pp);
	$corrects = array(strpos($letord, trim($last)));
	if ($corrects[0]===false) {
		echo "error on answer to $prompt";
		continue;
	}
...
header('Content-disposition: attachment; filename='.preg_replace('/[^\w]/','',$title).'.xml');
echo $out;
?>
